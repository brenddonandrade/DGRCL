...
class Encoder(nn.Module):
    def __init__(self, in_channels: int, out_channels: int, activation, base_model=GCNConv, k: int = 2, skip=False):
        ...
        self.k = k
        self.skip = skip
        # (in_channels, out_channels)*(k-2)块
        if not self.skip:
    ...
    def forward(self, x: torch.Tensor, edge_index: torch.Tensor):
        if not self.skip:
            for i in range(self.k):
                x = self.activation(self.conv[i](x.to(device='cuda:0'), edge_index))
            return x
        else:
...
class GRACE(torch.nn.Module):
    ...
    def loss(self, z1: torch.Tensor, z2: torch.Tensor, mean: bool = True, batch_size=50):
